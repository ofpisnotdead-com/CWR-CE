...
bool MemGrow::Commit(size_t size)
{
    ...
    if (_commited < size)
    {
        ...
        if (!commitOk)
        {
            ...
            (void)sysctlbyname("vm.swapusage", &sw, &len, nullptr, 0);
            ErrorMessage(
                "Cannot increase memory pool to %llu MB.\\n"
                "Current memory pool size is %llu MB.\\n"
                "Total swap: %llu MB, Free swap: %llu MB",
                static_cast<unsigned long long>(ConvertToMB(size)),
                static_cast<unsigned long long>(ConvertToMB(_commited)),
                static_cast<unsigned long long>(ConvertToMB((size_t)sw.su_totalpages * 4096ULL)),
                static_cast<unsigned long long>(ConvertToMB((size_t)(sw.su_totalpages - sw.su_usedpages) * 4096ULL)));
#else
            struct sysinfo si;
            sysinfo(&si);
            ErrorMessage("Cannot increase memory pool to %llu MB.\\n"
